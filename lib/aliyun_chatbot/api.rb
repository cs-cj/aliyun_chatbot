  ...
  module Api
    ...
    def public_params
      uuid = SecureRandom.uuid
      {
        ...
        Timestamp: Time.now.to_i, #	String	是	请求的时间戳。日期格式按照 ISO8601 标准表示，并需要使用UTC时间。格式为：YYYY-MM-DDThh:mm:ssZ，例如 2017-10-11T12:00:00Z （为北京时间 2017 年 10 月 11 日 20 点 0 分 0 秒）。
        SignatureVersion: 1.0, #	String	是	签名算法版本，目前版本是 1.0。
        SignatureNonce: uuid #"#{Time.now.to_i}#{rand(100..999)}" #,	String
      }
    end
    # module Some
    # end
    ...
    def chat_params( instance_id, utterance, session_id, knowledge_id)
      ...
      required.merge!({SessionId: session_id }) if session_id.present?
      
      #String	否	知识库词条的关联问题的ID。若指定此ID，那么机器人会直接返回指定的关联问题的答案
      required.merge!({KnowledgeId: knowledge_id }) if knowledge_id.present?
    ...
    def sender_params(sender_id, sender_nick, tag)
      ...
      optional.merge!({SenderId: sender_id }) if sender_id.present?
      #		String	否	当前会话中访问的昵称
      optional.merge!({SenderNick: sender_nick }) if sender_nick.present?
      #		String	否	可以传递任何内容参数，该参数将会直接在返回结果中透传回来。
      optional.merge!({Tag: tag }) if tag.present?
      optional
    end
  end
end
